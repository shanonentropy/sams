...
#import data matrix
fpath = 'C:\processed_data_sensor_2\data_matricies\svd_data_matrix\week3_laser_power_loop\demeaned_data_matrix_third_cycle_week_3_laser_power_loop'
ddf = dd.read_csv(fpath, sep=',', header=0)

ddf = ddf.rename(columns={'Unnamed: 0': 'temperature'});
#ddf.head(2)

client = Client(n_workers=1, threads_per_worker=1, memory_limit='100GB')
...
export_name = 'c:/sams//saved_data/sensor_2_week_3_laser_power_loop_screeplot.png'
# Create scree plot
...
df_pca.to_csv('c:/sams//saved_data/sensor_2_week_3_lsaer_power_pca_modes.csv')
...
export_name = 'c:/sams//saved_data/sensor_2_week_1_third_cyle_laser_power_eigenmode_{}.png'
...
export_name = 'c:/sams/saved_data/sensor_2_week_3_third_cycle_laser_power_eigenmode_{}_loading.png'
...
for i in range(n_comps):
    if i ==0:
        ...
        plt.savefig('c:/sams/saved_data/sensor_2_week_3_laser_power_loop_eigenmode_{}_against_first_mode.png'.format(i))
        plt.show()
...
x_t = loadings_train[:, 0:3]
lnr.fit(x_t, y_train)
y_pd = lnr.predict(x_t)
tr = np.sqrt(mean_squared_error(y_train, y_pd))
print(f'training error with 1 comps is {tr:3f}')
y_test_pd = lnr.predict(loadings_test[:, 0:7])
tr_t = np.sqrt(mean_squared_error(y_pred_test_comp, y_test_pd))
print(f'testing error with 1 comps is {tr_t:3f}')
...
plt.xlabel('Measured Laser_Power (percent power)')
plt.ylabel('Predicted Laser Power (percent power)')
...
df_.to_csv('n_comp_evaluation_third_cycle_laser_power.csv')
...
y_pred_testing =  ls.predict(loadings_test[:, 0:3])
...
plt.xlabel('Measured Laser_Power (percent power)')
plt.ylabel('Predicted Laser Power (percent power)')
plt.savefig('c:/sams/saved_data/sensor_2_week__third_cycle_RIDGE_testing.png', dpi = 700)
plt.show()
...
for a in alphas:
    ...
    y_pred_testing =  ridge_reg.predict(loadings_test[:, 0:])
...
plt.xlabel('pixel number')
plt.ylabel('Intensity (AU)')
plt.savefig('c:/sams/saved_data/sensor_2_week_1_second_cycle_mode_zero_sub_scaled_one.png', dpi = 700)
plt.show()
