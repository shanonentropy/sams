...
client = Client(n_workers=1, threads_per_worker=2, memory_limit='25GB')


delayed_results = [delayed(shredder.main_processor)(f) for f in shredder.filtered_files[::1000]]
...
shredder.svd_computation(f_save='test')
...
shredder.pca_regression( num_comps = 5, poly_deg=1)
