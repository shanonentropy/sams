...
time_of_measurement, set_temp, resistance = [],[],[] 

for _, temperature in enumerate(temp_index):
    print(temperature)
    drywell.set_temp(temperature)
    wait_for_x(sleep_seconds =30, timeout_seconds=2000)
    drywell.beep()
    t_start = time.monotonic()
    while time.monotonic()-t_start<36:
        a = np.fromstring((dmm.query(":READ?")).replace('\n',','), sep=',').mean()
        time_of_measurement.append(time.monotonic(), temperature,resistance.append(a) )
        sleep(10)
        print('coco')
...
data =  zip(time_of_measurement, set_temp, resistance)
...
with open(file_open, 'w', newline='') as f:
    ...
    for row in data:
        data_writer.writerow({'time_of_measurement': data[0], 'set_temp':data[1], 'resistance':data[2] })
